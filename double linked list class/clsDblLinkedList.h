...
public:
	...
	{
		if ((indx > (_Size - 1) || indx < 0))
			return NULL;

		Node* tempHead = head;
		while (tempHead != NULL && indx > 0)
		{
	...
	bool UpdateItem(int indx, T Value)
	{
		Node * theNode=GetNode(indx);
		if (theNode != NULL)
		{
			theNode->value = Value;
			return true;
